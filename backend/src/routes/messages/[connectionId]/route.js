...
export async function GET(req, { params }) {
  try {
    ...
    const messages = await messageService.getConnectionMessages(connectionId, userId, {
      limit,
      page,
      markAsRead,
    });
    ...
    return ok({
      messages,
      meta: { count: messages.length, connectionId, page, limit, markedAsRead: markAsRead },
    });
  } catch (error) {
    console.error("GET /api/messages/[connectionId] error:", error);
    if (error.message.includes("Connection not found")) return fail("Connection not found", 404);
    if (error.message.includes("Unauthorized")) return fail("You are not authorized to view messages in this connection", 403);
    if (error.message.includes("accepted connections")) return fail("Messages can only be viewed for accepted connections", 403);
...
export async function POST(req, { params }) {
  try {
    ...
    const { receiverId, content, messageType, attachments, replyTo } = await req.json();

    if (!receiverId || !content) return fail("Receiver ID and message content are required", 400);
    ...
    console.error("POST /api/messages/[connectionId] error:", error);
    if (error.message.includes("Connection not found")) return fail("Connection not found", 404);
