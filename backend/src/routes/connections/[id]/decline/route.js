...
export async function POST(req, { params }) {
  try {
    ...
    // Legacy alias route: keep supporting /decline while canonical state is "rejected".
    const connection = await connectionService.updateConnectionStatus(connectionId, userId, "rejected");
    return ok({ connection });
  } catch (error) {
    console.error("POST /api/connections/[id]/decline error:", error);
