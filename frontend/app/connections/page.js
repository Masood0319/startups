...
const ConnectionsPage = () => {
    ...
    useEffect(() => {
        ...
        fetchConnections();
    }, [activeTab, authLoading, user?._id]);

    const fetchConnections = async () => {
        setLoading(true);
        try {
            const status = activeTab === 'connections' ? 'accepted' : 'pending';
            const payload = await apiRequest(`connections?status=${status}`, { method: "GET" });
    ...
    const filteredConnections = connections.filter(conn => {
        if (activeTab === 'invitations') {
            return conn.status === 'pending' && conn.direction === 'inbound';
        }
        if (activeTab === 'sent') {
            return conn.status === 'pending' && conn.direction === 'outbound';
        }
...
const ConnectionCard = ({ connection, activeTab, onUpdate }) => {
            ...
            <div className="flex items-start justify-between">
                <div className="flex items-start space-x-4 flex-1">
                    ...
                    <div className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-xl">
                        {connection?.partnerName?.charAt(0) || 'U'}
                    </div>

                    {/* User Info */}
                    <div className="flex-1">
                        <h3 className="text-lg font-bold text-white">
                            {connection?.partnerName || 'Unknown User'}
                        </h3>
                        <p className="text-gray-400 text-sm">
                            {connection?.partnerRole || 'No role'}
                        </p>
