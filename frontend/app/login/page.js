            ...
            if (data.success) {
                try {
                    ...
                    if (me?.success && meUser) {
                        ...
                        if (meUser.role === 'investor') {
                            const targetPage = '/dashboard/investor'
                            console.log("Redirecting to:", targetPage)
                            router.push(targetPage)
                            return
                        }
                        if (meUser.role === 'founder') {
                ...
                setError(data.error || data.message || "Login failed") // ✅ show error
            }
    ...
    useEffect(() => {
        ...
        const check = async () => {
            try {
                ...
                if (active && data?.success && data?.data?.user) {
